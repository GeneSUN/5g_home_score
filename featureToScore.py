...
def calculate_DataScore(switch_count_sum, ServicetimePercentage, fiveg_usage_percentage): 

    weight_switch_count_sum = 0.2 
    
    weight_ServicetimePercentage = 0.5 
    
    weight_fiveg_usage_percentage = 0.3 

    if switch_count_sum is None and ServicetimePercentage is None and fiveg_usage_percentage is None: 
        return None 
    else: 
        total_weight = 0 
        score = 0 
        ...
        if ServicetimePercentage is not None: 
            score += weight_ServicetimePercentage * ServicetimePercentage 
            total_weight += weight_ServicetimePercentage 
...
def featureToScore(homeScore_df, feature, reverse=False): 
    ...
    lower5Col = percentile_approx(feature, 0.05, 100).over(windowSpec) 
    top95Col = percentile_approx(feature, 0.95, 100).over(windowSpec) 
...
def featureToScoreShfit(homeScore_df, feature): 
    ...
    # Create a new column by adding or subtracting the difference from the original feature 
...
if __name__ == "__main__":
    ...
    homeScore_df = homeScore_df.withColumn("ServicetimePercentage", col("ServicetimePercentage")*100 )\
    ...
    scaled_features = ["downloadresult","uploadresult","avg_CQI","log_avg_BRSRP","log_avg_SNR","log_avg_5GSNR"]
    scaled_features_reverse = ["latency"]
    raw_features = ["fiveg_usage_percentage"]
    ...
    df_score = df_score.withColumn("dataScore", F.round( calculate_DataScore("scaled_switch_count_sum", "scaled_ServicetimePercentage", "scaled_fiveg_usage_percentage"),2)  )
