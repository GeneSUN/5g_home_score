...
class CellularScore:
    ...
    def get_throughput_df(self, df_cust = None,df_price_cap = None):
        ...
        try:
            ...
            df_ultrag_price_cap = df_ultra.join(df_cust.select("IMSI","MDN_5G","PPLAN_CD","PPLAN_DESC","CPE_MODEL_NAME"), "IMSI", "right")\
                                            ...
                                            .withColumn(
                                                    "ULTRAGAUGE_DL_SCORE", 
                                                    F.round(F.when((F.col("UE_OVERALL_DL_SPEED") / F.col("DL_CAP") *2) > 1, 1)
            ...
            mail_sender.send(
                    ...
                    text=f"/fwa/npp_mdn_agg_insights_rtt/datadate={self.d}"
                )
        ...
        df_throughput = df_ultrag_price_cap.join(df_speedtest, "MDN_5G", "left")\
                                .withColumn(
                                    "Download_Score", 
                                    F.round(F.when((F.col("DOWNLOADRESULT") / F.col("DL_CAP")) > 1, 1)
                                     .otherwise(F.col("DOWNLOADRESULT") / F.col("DL_CAP")), 4) )\
                                .withColumn(
                                    "Upload_Score", 
    ...
    def get_linkCapacity_df(self, df_heartbeat = None):
        ...
        df_linkCapacity = df_with_bandwidths.filter(
                                        ...
                                        .withColumn(
                                            "_capacity", 
                                            F.round(
                                                100*((F.col("lte_capacity") + F.col("nw_capacity") + F.col("c_band_capacity")) / 218),
                                                2)
                                            )\
                                        .groupby("sn","imsi")\
                                        .agg( 
